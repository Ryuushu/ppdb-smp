const fs = require('fs');
let file = 'c:/laragon/www/ppdb-smp/resources/js/Pages/Admin/Settings/AdminItems.tsx';
let lines = fs.readFileSync(file, 'utf8').split(/\r?\n/);

// Find insertions:
// 1. Add `masterUkuranSeragams` to Props
let titleIdx = lines.findIndex(l => l.includes('title: string;'));
if (titleIdx !== -1 && !lines[titleIdx + 1].includes('masterUkuranSeragams: any[]')) {
    lines.splice(titleIdx + 1, 0, '\tmasterUkuranSeragams: any[];');
}

let propsIdx = lines.findIndex(l => l.includes('title,') && lines[l - 1]?.includes('items,'));
if (propsIdx !== -1 && lines[propsIdx + 1] && !lines[propsIdx + 1].includes('masterUkuranSeragams')) {
    lines.splice(propsIdx + 1, 0, '\tmasterUkuranSeragams,');
}

// 2. Add size items state
let stateIdx = lines.findIndex(l => l.includes('const [editId, setEditId] = useState'));
if (stateIdx !== -1 && !lines.find(l => l.includes('setEditUkuranId'))) {
    const sizeStateCode = `
...
`;
    lines.splice(stateIdx + 1, 0, sizeStateCode);
}
...
for (let i = lines.length - 1; i >= 0; i--) {
    if (lines[i].includes('</Card>')) {
        endCardIdx = i;
        break; // found the last </Card> (which is for standard admin items)
    }
}

if (endCardIdx !== -1 && !lines.some(l => l.includes("BAGIAN UKURAN BAJU"))) {
    const tableCode = `
				</div>

				{/* BAGIAN UKURAN BAJU */}
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t mt-8">
								...
								<Table>
									...
									<TableBody>
										{!masterUkuranSeragams || masterUkuranSeragams.length === 0 ? (
											...
											masterUkuranSeragams.map((item: any) => (
...
`;
    lines.splice(endCardIdx + 1, 0, tableCode);
}

// 4. Update import
let importIdx = lines.findIndex(l => l.includes('import { Plus, Trash2, Receipt, Pencil, X } from "lucide-react";'));
if (importIdx !== -1) {
    lines[importIdx] = 'import { Plus, Trash2, Receipt, Pencil, X, Shirt } from "lucide-react";';
}
